...
#define MOVEMENT_DURATION	2 /* seconds */
...
static int duration_to_lsb(struct lis3dh_data *lis, int duration, u8 *lsb)
{
...
static int lis3dh_write_config(struct lis3dh_data *lis, u8 config)
{
	...
	buf[0] = INT1_DURATION;
	err = duration_to_lsb(lis, MOVEMENT_DURATION, &buf[1]);
	if (err < 0)
		return err;
	err = lis3dh_i2c_write(lis, buf, 1);
	if (err < 0)
